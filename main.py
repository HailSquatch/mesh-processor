...
app = FastAPI(title="StormDataPro MESH Processor", version="0.5.2")
...
# Consensus scoring thresholds — calibrated 2026-04-19 for spring storm reality.
# Original Phase 5 thresholds were tuned for summer supercells and stripped many
# real spring/April storms (echo tops typically 8-11 km, not 12+).
CONSENSUS_POSH_MIN = 40            # POSH ≥ 40% contributes (was 50% - too strict for fast storms)
CONSENSUS_REFL_DBZ_MIN = 50        # Reflectivity@-20C ≥ 50 dBZ contributes (was 55 - peak misses on fast cells)
CONSENSUS_ECHO_TOP_KM_MIN = 9.0    # ~30 kft; reasonable for spring storms (was 12.2 / 40kft - summer level)
CONSENSUS_MIN_SCORE = 40           # pixel must score ≥ this to draw polygon (was 50 - too strict, dropped real storms)
    ...
    def run(self):
        ...
        try:
            ...
            for band in POLYGON_BANDS:
                ...
                # Consensus scoring — calibrated to handle missing data gracefully.
                # MESH (40): always present where bands match
                # POSH (30 or 0): straightforward threshold
                # Reflectivity (20, 10, or 0): give half-credit for missing data (e.g. outside CONUS)
                #   instead of zero. A real storm shouldn't be docked because we don't have refl coverage.
                # Echo Top (10 or 0): straightforward threshold
                ...
                # Reflectivity: 20 pts if ≥ threshold, 10 pts if missing (sentinel 0), 0 pts if low
                ...
                crop_mesh_meets = crop_score = refl_missing = refl_strong = None
                gc.collect()
                ...
                # Morphology — tiered by band to balance noise reduction with detection.
                # Smaller hail (1.0-1.5"): aggressive denoising since false positives common
                # Medium hail (1.5-2.0"): light denoising
                # Severe hail (2.0"+): NO opening - real 2"+ cores are rare, every pixel matters.
                #   Closing still applied to fill gaps in the hail track.
                if band["min_inches"] >= 2.0:
                    # Severe hail: only close gaps, don't strip small cores
                    cleaned = ndimage.binary_closing(pass_mask, iterations=2)
                elif band["min_inches"] >= 1.5:
                    # Medium hail: light open + close
                    cleaned = ndimage.binary_opening(pass_mask, iterations=1, structure=np.ones((2,2)))
                    cleaned = ndimage.binary_closing(cleaned, iterations=2)
                else:
                    # Light hail: full open + close (current behavior - denoise more)
                ...
                for poly in simplified:
                    ...
                    if sub_c1 > sub_c0 and sub_r0 > sub_r1:
                        ...
                        if sub_mask.any():
                            ...
                            # Refl: full credit if strong, half credit if missing (no coverage)
                            if peak_refl_x10 >= refl_dbz_x10_thresh:
                                sc += 20
                            elif peak_refl_x10 == 0:
                                sc += 10  # half credit for missing data
...
class Archiver:
    ...
    def _fetch_spc_today(self) -> dict:
        ...
        try:
            ...
            return self._parse_spc_csv(r.text)
        except Exception as e:
            log.warning(f"SPC fetch failed: {e}")
            return {"type": "FeatureCollection", "features": [], "error": str(e)}

    def _parse_spc_csv(self, csv_text: str) -> dict:
...
def startup():
    scheduler.start()
    log.info("Phase 5.2 scheduler: tick 2min (5 products), polygonize 5min, midnight CT reset, 1440 sanity hourly, archive retention 6 weeks")
...
def root():
    return {
        ...
        "version": "0.5.2",
        "phase": "5.2 — historical archive + midnight CT reset",
