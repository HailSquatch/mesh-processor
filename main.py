...
class Archiver:
    ...
    def save_snapshot(self, archive_date_str: str) -> dict:
        ...
        try:
            ...
            # Also snapshot the heatmap PNG + meta so the frontend can show
            # per-day heatmaps when browsing archive dates.
            heatmap_archived = False
            try:
                if os.path.exists(HEATMAP_PNG_FILE):
                    import shutil
                    shutil.copy2(HEATMAP_PNG_FILE,
                                 os.path.join(ARCHIVE_DIR, f"{archive_date_str}_heatmap.png"))
                if os.path.exists(HEATMAP_META_FILE):
                    import shutil
                    shutil.copy2(HEATMAP_META_FILE,
                                 os.path.join(ARCHIVE_DIR, f"{archive_date_str}_heatmap_meta.json"))
                heatmap_archived = True
            except Exception as he:
                log.warning(f"Heatmap archive failed (non-fatal): {he}")

            log.info(f"Archived {archive_date_str}: {poly_count} polygons, {gt_count} ground-truth reports, heatmap={heatmap_archived} → {path}")
...
def history_heatmap_png(date_str: str):
    """Return the archived heatmap PNG for a specific storm day."""
    ...
    return FileResponse(path, media_type="image/png", headers={
        "Cache-Control": "public, max-age=86400",  # archive never changes — cache 24h
    })


@app.get("/history/{date_str}/heatmap/meta")
def history_heatmap_meta(date_str: str):
    """Return the archived heatmap bounds metadata for a specific storm day."""
    path = os.path.join(ARCHIVE_DIR, f"{date_str}_heatmap_meta.json")
    if not os.path.exists(path):
        return {"bounds": None, "size": None, "note": f"No archived heatmap metadata for {date_str}"}
    try:
