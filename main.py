...
BANDS = [
    # (min_in, label,        (R,   G,   B,   A))
    (0.75, "0.75\u20131\"",  (96, 190, 110, 88)),
    (1.00, "1\u20131.25\"",  (255, 222, 66, 96)),
    (1.25, "1.25\u20131.5\"",(255, 180, 40, 104)),
    (1.50, "1.5\u20131.75\"",(255, 128, 24, 112)),
    (1.75, "1.75\u20132\"",  (250, 76, 28, 120)),
    (2.00, "2\u20132.5\"",   (222, 28, 28, 128)),
    (2.50, "2.5\u20133\"",   (168, 12, 60, 132)),
    (3.00, "3\u20134\"",     (128, 16, 128, 136)),
    (4.00, "4\"+",           (72, 8, 96, 140)),
]
    ...
    for url in (SPC_URL, SPC_URL_FALLBACK):
        ...
        for line in lines[1:]:
            ...
            out.append({
                ...
                "source": "SPC",
                "place": p[i_loc].title() if i_loc >= 0 and i_loc < len(p) else "",
                "time": p[i_time] if i_time >= 0 and i_time < len(p) else "",
            })
        if out:
            break
    return out
...
def render_heatmap(mesh_in: np.ndarray, lat_n: float, lat_s: float,
    ...
    """Calibrated inches grid -> cropped RGBA PNG bytes + bounds meta.
    Crops to the bounding box of displayable hail (with padding)."""
    ny, nx = mesh_in.shape
    dlat = (lat_n - lat_s) / (ny - 1) if ny > 1 else 0.01
    dlon = (lon_e - lon_w) / (nx - 1) if nx > 1 else 0.01

    mask = mesh_in >= MIN_INCH
    if not mask.any():
        img = Image.new("RGBA", (2, 2), (0, 0, 0, 0))
        buf = io.BytesIO()
        img.save(buf, format="PNG")
        return buf.getvalue(), None, {"cells": 0, "max_in": 0.0}
    ...
    for min_in, _label, color in BANDS:
        rgba[crop >= min_in] = color
    rgba[crop < MIN_INCH] = (0, 0, 0, 0)
    ...
    stats = {
        "cells": int(mask.sum()),
        "max_in": round(float(crop.max()), 2),
        "size_px": [int(crop.shape[1]), int(crop.shape[0])],
    }
