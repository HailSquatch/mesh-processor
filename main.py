...
Env vars (all optional):
  ...
  MESH_60_URL, SPC_URL, LSR_URL, IEM_MESH_BASE, SPC_DATED_BASE   source overrides
...
def render_products(mesh_in: np.ndarray, lat_n: float, lat_s: float,
    ...
    features = []
    for min_in, label, color in BANDS:
        if csm.max() < min_in:
            continue
...
def iem_hour_url(ts_utc):
    return (f"{IEM_MESH_BASE}/{ts_utc.year}/{ts_utc.month:02d}/{ts_utc.day:02d}"
            f"/mrms/ncep/MESH_Max_60min/MESH_Max_60min_00.50_"
            f"{ts_utc.strftime('%Y%m%d-%H%M%S')}.grib2.gz")
...
def merge_hourly(day, until_utc=None):
    ...
    for ts in local_midnight_utc_hours(day):
        ...
        for delta_min in (0, 2, -2):
            try:
                vals, lat_n, lat_s, lon_w, lon_e, _vt = fetch_mesh_grid(
                    iem_hour_url(ts + timedelta(minutes=delta_min)))
                ok = True
                break
            except Exception:
                continue
...
def cycle():
    ...
    if grid is not None and sdate and sdate != str(today_local):
        try:
            archive_today_state(grid, geo, sdate, reports)
        except Exception as e:
