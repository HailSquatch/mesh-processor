...
app = FastAPI(title="StormDataPro MESH Processor", version="0.9.0")
...
# We generate a continuous color ramp from the MESH grid directly. The bands here
# match HailPoint's published color scale (0.75/1.00/1.25/1.50/1.75/2.00/2.25/2.50/2.75/3.00/3.50/4.00").
# Each tuple is (size_inches, R, G, B, alpha 0-255).
HEATMAP_COLOR_STOPS = [
    (0.75, 255, 244, 154, 110),    # pale yellow, semi-transparent edge
    (1.00, 255, 220, 100, 165),    # yellow
    (1.25, 255, 184, 60,  185),    # gold
    (1.50, 255, 140, 40,  200),    # light orange
    (1.75, 255, 100, 30,  210),    # orange
    (2.00, 230, 50,  30,  220),    # red-orange
    (2.25, 200, 25,  35,  225),    # red
    (2.50, 160, 15,  35,  230),    # dark red
    (2.75, 220, 60,  220, 230),    # magenta
    (3.00, 240, 130, 240, 235),    # bright pink
    (3.50, 230, 230, 230, 240),    # near-white
    (4.00, 255, 255, 255, 245),    # white
]
HEATMAP_MIN_INCHES = 0.75   # below this, fully transparent
HEATMAP_DOWNSAMPLE = 2      # 2 = output ~2km pixels (good balance: detail vs file size)
                            # 1 = full 1km resolution; 4 = ~4km, smaller file
...
class Polygonizer:
    ...
    def _generate_heatmap(self, crop_mesh: np.ndarray, gr0: int, gr1: int, gc0: int, gc1: int):
        ...
        try:
            ...
            # Step 4: apply a light Gaussian blur to alpha channel for soft edges (the
            # topographic look). We blur ONLY alpha so colors stay crisp at their exact value
            # but edges fade smoothly. Sigma=0.7 gives the HailPoint-like gradient feel.
            try:
                from scipy.ndimage import gaussian_filter
                rgba[..., 3] = gaussian_filter(rgba[..., 3], sigma=0.7).astype(np.uint8)
            except Exception:
                pass  # blur is cosmetic; fine to skip if scipy missing
...
def root():
    return {
        ...
        "version": "0.9.0",
        "phase": "9.0 — HailPoint-style topographic heatmap PNG",
