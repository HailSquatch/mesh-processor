...
class Polygonizer:
    ...
    def run(self):
        ...
        try:
            # Step 1: find the bbox of ALL hail pixels (at any threshold) using a cheap read
            # We use the 1" threshold since that's the smallest — any polygon we'd draw
            # must have MESH >= 1" somewhere inside it
            min_thresh_x10 = int(POLYGON_THRESHOLDS[0]["min_mm"] * SCALE)
            ...
            if not rows_any.any():
                ...
                new_geojson = {
                    ...
                    "metadata": {
                        ...
                        "note": "No hail at or above 1\" anywhere in window",
                    },
                    "features": [],
                }
                with self.lock:
            ...
            for thresh in POLYGON_THRESHOLDS:
                thresh_x10 = int(thresh["min_mm"] * SCALE)
                crop_mesh_meets = crop_mesh >= thresh_x10
...
def startup():
    scheduler.start()
    log.info("Phase 5 scheduler: tick 2min (5 products), polygonize 5min, 1440 sanity hourly")
